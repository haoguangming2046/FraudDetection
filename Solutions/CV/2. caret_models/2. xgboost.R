...
save(cv_perf, param0, best_nrounds, file=paste0(out_dir, "/xgboost.RData"))
...
png(filename = paste0(out_dir, "/2. xgboost roc.png"), width=480, height=480)
ggroc(auc_rf)
dev.off()
...
cutoff[which.min(test_acc_diff)]

# Variable Importance
importance_matrix <- xgb.importance(feature_names, model=finalFit)
png(filename = paste0(out_dir, "/2. xgboost var importance.png"), width=480, height=480)
...
save(best_nrounds, final_cv_pred, testPred, file=paste0(out_dir, "/ouput_xgboost.RData"))
